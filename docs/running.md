...
python testmqtt.py longvideo.mov   # q หรือ ESC = ออก, space = หยุดชั่วคราว
```

รันโดยไม่เปิดหน้าต่างแสดงผล

```bash
python testmqtt.py longvideo.mov --no-show
```
...
|---|---|---|
| `video` | `longvideo.mov` | ไฟล์วิดีโอ, RTSP URL, หรือเลขกล้อง (`0` = webcam) |
...
```bash
python testmqtt.py longvideo.mov --loop
```
...
ความหมายของ `congestion_level` ดูตารางใน [README](../README.md#ข้อมูลที่ส่ง)
ไม่มีรถใน ROI เลยก็ยังได้ 0 ไม่ใช่ null เพราะถนนว่างคือถนนที่ไม่ติด
...
สคริปต์ assert ธรรมดา ไม่ต้องลง pytest ใช้ตรวจ payload, feature และการจับคู่
สถานะจริงที่เวลาอีก 5 นาที
